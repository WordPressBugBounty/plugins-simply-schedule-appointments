...
/**
 * Class TD_Health_Check_Debug_Data
 */
class TD_Health_Check_Debug_Data {
	...
	static function debug_data( $locale = null ) {
		...
		if ( method_exists( $wpdb, 'db_version' ) ) {
			if ( $wpdb->use_mysqli ) {
				...
				$server = mysql_get_server_info( $wpdb->dbh ); // phpcs:ignore
			}
		} else {
			$server = null;
		}
		...
		if ( isset( $wpdb->use_mysqli ) && $wpdb->use_mysqli ) {
			...
			if ( preg_match( '|[0-9]{1,2}\.[0-9]{1,2}\.[0-9]{1,2}|', mysql_get_client_info(), $matches ) ) { // phpcs:ignore
		...
		$info['wp-install-size']['fields'] = TD_Health_Check_Debug_Data::get_installation_size();
	...
	public static function get_installation_size() {
		...
		foreach ( $sizes as $size => $attributes ) {
			try {
				$sizes[ $size ]['size'] = TD_Health_Check_Debug_Data::get_directory_size( $attributes['path'] );
			} catch ( Exception $e ) {
				$inaccurate = true;
			}
		}

		$size_db = TD_Health_Check_Debug_Data::get_database_size();
