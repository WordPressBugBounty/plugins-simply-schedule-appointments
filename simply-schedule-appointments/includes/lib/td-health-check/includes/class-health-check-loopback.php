...
/**
 * Class TD_Health_Check_Loopback
 */
class TD_Health_Check_Loopback {
	/**
	 ...
	 * @uses rand()
	 * @uses update_option()
	 * @uses TD_Health_Check_Loopback::can_perform_loopback()
	 * @uses sprintf()
	 * @uses esc_attr()
	 * @uses esc_html__()
	 * @uses esc_html()
	...
	static function loopback_no_plugins() {
		...
		if ( ! TD_Health_Check_Troubleshoot::mu_plugin_exists() ) {
			if ( ! TD_Health_Check::get_filesystem_credentials() ) {
				...
				$check_output = TD_Health_Check_Troubleshoot::setup_must_use_plugin();
		...
		$no_plugin_test = TD_Health_Check_Loopback::can_perform_loopback( $loopback_hash );
	...
	static function loopback_test_default_theme() {
		...
		$theme_test = TD_Health_Check_Loopback::can_perform_loopback( $loopback_hash, '' );
